...
describe("fetch", () => {
  ...
  it("should fetch with default remote", async () => {
    ...
    deepEqual(executeGitCommandMock.mock.calls[0].arguments[0], ["fetch", "origin"]);
  });
  ...
  it("should fetch with custom remote", async () => {
    ...
    ok(result.ok);
    deepEqual(executeGitCommandMock.mock.calls[0].arguments[0], ["fetch", "upstream"]);
  });
  ...
  it("should pass cwd option", async () => {
    ...
    equal(executeGitCommandMock.mock.calls[0].arguments[1].cwd, "/path/to/repo");
  });
